...
void RosAriaNode::publish() {
    ...
    for (int i = 0; i < position.pose.covariance.size(); i++) {
        position.pose.covariance.c_array()[i] = MyCovariance[i];
    }
    for (int i = 0; i < position.pose.covariance.size(); i++) {
        position.twist.covariance.c_array()[i] = MyCovariance[i];
    }
...
RosAriaNode::cmdvel_cb(const geometry_msgs::TwistConstPtr &msg) {
    ...
    ROS_DEBUG("RosAria: sent vels to to aria (time %f): x vel %f mm/s, y vel %f mm/s, ang vel %f deg/s",
              veltime.toSec(),
              (double) msg->linear.x * 1e3, (double) msg->linear.y * 1.3, (double) msg->angular.z);
}
